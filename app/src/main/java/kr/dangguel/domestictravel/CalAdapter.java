...
public class CalAdapter extends BaseAdapter {
    ...
    public CalAdapter(LayoutInflater inflater, ArrayList<SaveCalVO> cals, MainActivity mainActivity) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        tvTitle.setText(saveCalVO.title);
        tvRange.setText(saveCalVO.range);
        tvDays.setText(saveCalVO.days);
        tvUntilday.setTextColor(Color.RED);
        if (saveCalVO.untilday < 0) {
            tvUntilday.setTextColor(Color.GRAY);
            tvUntilday.setText("완료된 여행입니다");
            iv.setVisibility(View.GONE);
        } else tvUntilday.setText(saveCalVO.untilday + " 일 남음");
